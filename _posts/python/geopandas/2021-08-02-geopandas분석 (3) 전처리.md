...
이번에는 좌표변환해준 geojson과 생활인구 데이터를 Merge시켜주는 작업을 진행합니다. 생활인구 데이터 (local_people)에 통계청행정동코드를 이용하여 위치 merge를 진행합니다. 이것도 앞서 언급했던 것처럼, astype()을 이용하여 int에서 str로 변환해줍니다. 
...
행정동 이름에 맞게 윛(위도, 경도)가 잘 붙여진 것을 볼 수 있다. 혹시나 위치가 안붙은게 있나 확인하는 작업을 해주기 위해서 넣은 코드이다.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzYxNTAwNTQ2XX0=
-->