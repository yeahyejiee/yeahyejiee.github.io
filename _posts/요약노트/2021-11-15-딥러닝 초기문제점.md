...
- weight를 0으로 두면 학습이 이루어지지않는 경우가 발생한다.
	...
	- xaiver: $입력값~출력값 사이 임의의 수 \over 출력/입력값^2$ $\Longrightarrow$  $입력값~출력값 사이 임의의 수 \over 출력/(입력값/2)^2$ 
	- sigmoid, tanh $\to$ Xavier,ReLU $\to$ H
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYxMzAwMzg0MSw2MzcxMzEyODksLTE1MT
U3MDg1MTZdfQ==
-->